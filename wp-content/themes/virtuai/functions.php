...
function virtuai_register_components($components) {
...
Hooks::prefixed_add_filter('components', 'virtuai_register_components', 20);
Theme::load(
	array(
 ...
 * @return Theme
 */
function virtuai_theme() {
	return Theme::getInstance();
}

function virtuai_assets() {
	return virtuai_theme()->getAssetsManager();
}
...
virtuai_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__('Header Menu', 'virtuai'),
			'footer-menu' => esc_html__('Footer Menu', 'virtuai'),
		)
	);
...
if (!apply_filters('kubio_is_enabled', false)) {
	virtuai_assets()
		...
		->registerStylesheet('virtuai-theme', '/theme/theme.css');
	virtuai_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		virtuai_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action('wp_enqueue_scripts', 'virtuai_print_color_scheme', 0);
} else {
	virtuai_assets()
		->registerStyle('virtuai-theme', virtuai_assets()->getBaseURL() . '/theme/fse-base-style.css');
	Hooks::prefixed_add_filter('skip_google_fonts', '__return_true');
}

function virtuai_add_woocommerce_suppoert() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action('after_setup_theme', 'virtuai_add_woocommerce_suppoert');
...
Hooks::add_wp_ajax(
	...
	function () {
		...
		Flags::set('start_source', $with_ai === 'yes' ? "notice-ai" : "notice-homepage");
...
function virtuai_get_builder_plugin_slug() {
...
function virtuai_theme_plugins($plugins) {

	$plugin_slug = virtuai_get_builder_plugin_slug();

	$plugins[$plugin_slug] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __('Kubio PRO', 'virtuai') : __('Kubio', 'virtuai'),
...
Hooks::prefixed_add_filter('theme_plugins', 'virtuai_theme_plugins');

Hooks::prefixed_add_action(
	'after_plugin_activated',
	function ($slug) {

		if ($slug === virtuai_get_builder_plugin_slug()) {
			...
			<div class="kubio-admin-row get-started-imported notice notice-success">
				<div>
					<p class="imported-title">
						...
						echo esc_html(
							sprintf(
								__('%s design has been successfully imported!', 'virtuai'),
								virtuai_theme()->getName()
							)
						);
						?>
					</p>
					<p class="imported-subtitle">
						...
						echo esc_html(
							sprintf(
								__('%s design has been successfully imported! You can take a look at your new design or start editing it', 'virtuai'),
								virtuai_theme()->getName()
							)
						);
						?>
				...
				<div class="button imported-view-site-button">
					<a href="<?php echo esc_url(site_url()); ?>">
						<?php echo esc_html(__('View site', 'virtuai')); ?>
					</a>
...
function virtuai_try_online_url() {
	return "https://kubiobuilder.com/go/try-theme/" . get_template();
}

add_filter('kubio/editor-try-online/url', 'virtuai_try_online_url', 10);

function virtuai_render_header_style() {

	virtuai_theme()->get('css')->render();
}
add_action('wp_head', 'virtuai_render_header_style', 100);
