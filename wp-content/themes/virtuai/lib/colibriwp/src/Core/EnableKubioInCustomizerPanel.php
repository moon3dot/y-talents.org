...
class EnableKubioInCustomizerPanel extends \WP_Customize_Panel {
	...
	protected function render() {
		...
		$message = __( 'The Kubio plugin takes VirtuAI to a whole new level by adding new and powerful editing and styling options. Wanna have full control over your design with VirtuAI?', 'virtuai' );
		...
		<?php if ( $this->getPluginState( $this->plugin_slug ) !== PluginsManager::ACTIVE_PLUGIN ) : ?>
			...
			<?php if ( $this->getPluginState( $this->plugin_slug ) === PluginsManager::NOT_INSTALLED_PLUGIN ) : ?>
				<button data-colibri-plugin-action="install" class="button button-primary kubio-open-editor-panel-button ">
					<span data-action="install"><?php esc_html_e( 'Install Kubio', 'virtuai' ); ?></span>
				</button>
			<?php elseif ( $this->getPluginState( $this->plugin_slug ) === PluginsManager::INSTALLED_PLUGIN ) : ?>
				<button data-colibri-plugin-action="activate" class="button button-primary kubio-open-editor-panel-button">
					<span data-action="activate"><?php esc_html_e( 'Activate Kubio', 'virtuai' ); ?></span>
				</button>
			<?php endif; ?>
			</div>
		<?php endif; ?>
