...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".virtuai-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.virtuai_admin) {
    return;
  }

  var _window$virtuai_admin = window.virtuai_admin,
      builderStatusData = _window$virtuai_admin.builderStatusData,
      getStartedData = _window$virtuai_admin.getStartedData;
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".virtuai-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.virtuai-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".virtuai-admin-big-notice").addClass("processing");
      $(".virtuai-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".virtuai-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.virtuai-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.virtuai-admin-big-notice').on("click", ".virtuai-notice-dont-show-container", disableNotice);
  ...
  window.virtuai_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
